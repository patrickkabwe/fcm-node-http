...
export const ErrorCode = {
  ...
  UNREGISTERED:
    "The registration token is not valid for the target project. This can happen if the token is: 1) Invalid. 2) Associated with a different project. 3) Not registered with FCM. 4) Unregistered from FCM. 5) Deleted from the device.",
  SENDER_ID_MISMATCH:
    "The registration token is not valid for the target project. This can happen if the token is: 1) Invalid. 2) Associated with a different project. 3) Not registered with FCM. 4) Unregistered from FCM. 5) Deleted from the device.",
  QUOTA_EXCEEDED:
...
export const handleAxiosError = (error: any) => {
  if (error.response?.data?.error?.details?.length) {
    ...
    const code = details[1]?.errorCode as ErrorType;
